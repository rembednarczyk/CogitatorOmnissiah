...
- **Frontend** (`/src/`): components in `src/components/` (atomic parts in subdirs), all data fetching via custom hooks in `src/hooks/`. `useSyncManager` owns cross-ritual orchestration; `useSync` is the standard single-stream SSE pattern (built on `src/utils/sse.ts` `consumeSSE` + `stallWatchdog`) — reuse it, don't re-implement.
